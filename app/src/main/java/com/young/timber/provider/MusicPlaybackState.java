package com.young.timber.provider;
...
public class MusicPlaybackState {
    private static MusicPlaybackState mInstance = null;
    ...
    public static final synchronized MusicPlaybackState getInstance(final Context context) {
        if (mInstance == null) {
            mInstance = new MusicPlaybackState(context.getApplicationContext());
        }
        return mInstance;
    }
    ...
    public synchronized void saveState(final ArrayList<MusicPlaybackTrack> queue, LinkedList<Integer> history) {
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    for (int i = 0; iterator.hasNext() && i < NUM_PROCESS; i++) {
                        ...
                        contentValues.put(PlaybackHistoryColumns.POSITION, iterator.next());
                        database.insert(PlaybackHistoryColumns.NAME, null, contentValues);
                    }
                    database.setTransactionSuccessful();
                } catch (Exception e) {
                    e.printStackTrace();
